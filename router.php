...
function rand_podstav_smv($text){
      //$text = str_replace('х','x',$text);//исп
     $text = str_replace('а','a',$text);
    //$text = str_replace('р','p',$text);//исп
   $text = str_replace('о','o',$text);
   //$text = str_replace('е','e',$text);//исп
    //$text = str_replace('с','c',$text);//исп
    return $text;
}
...
$mainContent = str_replace('Москва, ул.Сретенка, д.27/29, стр 1', 'г. Санкт-Петербург, ул. Академика Лебедева, дом № 12, oф. 169-170', $mainContent  );
...
$mainContent = str_replace('http://studotzyv.ru/rel.php?id=aHR0cDovL3d3dy5yb3NkaXBsb20ucnU=', 'http://knowbaza.ru/', $mainContent  );
$mainContent = str_replace('http://studotzyv.ru/rel.php?id=aHR0cDovL3Jvc2RpcGxvbS5ydS96YWthei9mYXN0Lz9yZWY9c3R1ZG90enl2', 'http://knowbaza.ru/order.php', $mainContent  );
...
<meta name="yandex-verification" content="3e2a848a6520cb98" />
<meta name="google-site-verification" content="9g_kdGXJJ93pKqycxRskoayroi-porC7bRaKAnOkBLo" />', $mainContent  );
...
$mainContent = str_replace('Москва, ул.Сретенка, д.27/29, стр 1','Санкт-Петербург, ул.Академика Лебедева, д.12', $mainContent  );
...
$mainContent = str_replace("55.753994, 37.622093", "59.962086, 30.355053", $mainContent  );
